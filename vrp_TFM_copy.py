...
def get_data_from_sql():
    ...
    NODO_BASE = "A00010"
    visits_list.append({'loc_id': NODO_BASE, 'start': 0, 'end': 1440, 'type': 'depot', 'proceso': 'BASE', 'mce': 0})
    ...
    for cliente_id, group in df_v.groupby('CLIENTE_ID'):
        ...
        # Obtenemos la carga consolidada de la nueva tabla
        carga_real = mce_lookup.get(id_limpio, 0)
        
        # Extraemos todas las ventanas de este cliente con su proceso asociado
        client_windows = []
        for _, row in group.iterrows():
            client_windows.append({
                'win': [convertir_hora_a_minutos(row['MINIMO']), convertir_hora_a_minutos(row['MAXIMO'])],
                'proceso': row['PROCESO_ID']
            })
        
        client_windows.sort(key=lambda x: x['win'][0]) # Ordenamos por hora de inicio

        if not client_windows: continue

        # Lógica de fusión/separación de ventanas (CONSERVADA)
        merged = []
        curr = client_windows[0]
        
        for next_win in client_windows[1:]:
            if next_win['win'][0] <= curr['win'][1]: # Si se solapan, hacemos intersección (lo más restrictivo)
                curr['win'][0] = max(curr['win'][0], next_win['win'][0])
                curr['win'][1] = min(curr['win'][1], next_win['win'][1])
                curr['proceso'] += f" + {next_win['proceso']}"
    ...
    for loc in pickup_locs:
        if loc != NODO_BASE:
            visits_list.append({'loc_id': loc, 'start': 0, 'end': 1440, 'type': 'pickup', 'proceso': 'RECOGIDA', 'mce': 0})
...
def generate_map(data, manager, routing, solution):
    ...
    # IDs de tiendas especiales a resaltar
    TIENDAS_ESPECIALES = ['C03940', 'C31074']
    ...
    for vehicle_id in range(data["num_vehicles"]):
        ...
        while not routing.IsEnd(index):
            ...
            # --- AJUSTE DE COORDENADAS PARA EVITAR SOLAPE ---
            ...
            popup_content = f"""
                <div style="min-width: 180px; font-family: Arial, sans-serif;">
                    ...
                    <b>⏰ Hora Llegada:</b> {time_val//60:02d}:{time_val%60:02d}<br>
            ...
            # --- LÓGICA DE ICONOS (INCLUYENDO TIENDAS ESPECIALES) ---
            if node_index == data['depot']:
                ...
                # Resaltamos tiendas especiales con color naranja y estrella
...
def main():
    ...
    routing.AddDimension(time_callback_index, 120, 1440, False, "Time")
    time_dimension = routing.GetDimensionOrDie("Time")
    ...
    if solution:
        ...
        print(f"⏱️ Tiempo Total Proceso: {end_time_total - start_time_total:.2f}s")
